...
void loop()
{
  ...
  uint8_t count = countBinary(binaryReadings);
  switch (count)
  {
  case 1: // 
    if (binaryReadings == 0b00001)
      motor_move(-(0.75 * SPEED), SPEED, 1);  
    // Serial.println("turn right");  
    else if (binaryReadings == 0b10000)
      motor_move(SPEED, -(0.75 * SPEED), 0);  
    // Serial.println("turn left"); 
    else if (binaryReadings == 0b00100)
      motor_move(SPEED, SPEED, 2);  
    // Serial.println("forward"); 
    break;

  case 2:
    if (binaryReadings == 0b11000)
      motor_move(-(0.50 * SPEED), SPEED, 0);  
    // Serial.println("turn right");  

    else if (binaryReadings == 0b00011)
      motor_move(SPEED, -(0.50 * SPEED), 1);  
    // Serial.println("turn left"); 

    else if (binaryReadings == 0b01100)
      motor_move(-(0.25 * SPEED), SPEED, 0);  
    // Serial.println("turn right");  

    else if (binaryReadings == 0b00110)
      motor_move(SPEED, -(0.25 * SPEED), 1);  
    // Serial.println("turn left"); 

    break;

  case 3:
    if (binaryReadings == 0b01110)
      motor_move(SPEED, SPEED, 2);  
    // Serial.println("forward"); 
